...
class PolicyGradient(LFramework):
    ...
    def loss(self, mini_batch):
        ...
        e1_unique = torch.unique(e1)
        e2_unique = torch.unique(e2)
        r_unique = torch.unique(r) 
        e1_embeding_encoder = self.kg.x[e1_unique]
        e2_embeding_encoder = self.kg.x[e2_unique] 
        r_embedding_encoder = self.kg.y[r_unique] 
        e1_embedding_decoder = self.kg.entity_embeddings.weight[e1_unique]
        e2_embeding_decoder = self.kg.entity_embeddings.weight[e2_unique]
        r_embedding_decoder = self.kg.relation_embeddings.weight[r_unique]  
        cos_sim_e1_encoder_decoder = F.cosine_similarity(e1_embeding_encoder, e1_embedding_decoder, dim=-1)
        cos_sim_e2_encoder_decoder = F.cosine_similarity(e2_embeding_encoder, e2_embeding_decoder, dim=-1)
        cos_sim_r_encoder_decoder = F.cosine_similarity(r_embedding_encoder, r_embedding_decoder, dim=-1)
        cosine_similarity_regularization = 1.0 - cos_sim_e1_encoder_decoder.mean() + 1.0 - cos_sim_e2_encoder_decoder.mean() + 1.0 - cos_sim_r_encoder_decoder.mean()
        # print(e2_embeding_encoder.shape, r_embeding_decoder.shape)
        # print(e1_embeding,e1_embeding.shape) # 正常获取
        ...
        loss_dict['regular'] = cosine_similarity_regularization
