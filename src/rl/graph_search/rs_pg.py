...
class RewardShapingPolicyGradient(PolicyGradient):
    def __init__(self, args, kg, pn, fn_kg, fn, fn_secondary_kg=None):
        ...
        if fn_model in ['conve']:
            fn_state_dict = torch.load(args.conve_state_dict_path, map_location='cuda:%d' % self.args.gpu)
            ...
            fn_state_dict = torch.load(args.distmult_state_dict_path, map_location='cuda:%d' % self.args.gpu)
            fn_kg_state_dict = get_distmult_kg_state_dict(fn_state_dict)
        elif fn_model == 'complex':
            fn_state_dict = torch.load(args.complex_state_dict_path, map_location='cuda:%d' % self.args.gpu)
        ...
        if fn_model == 'hypere':
            complex_state_dict = torch.load(args.complex_state_dict_path,map_location='cuda:%d' % self.args.gpu)
