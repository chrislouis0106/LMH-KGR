...
def initialize_model_directory(args, random_seed=None):
    ...
    initialization_tag,
    hyperparam_sig,)
    # path_encoder_type 这个需要区分；number_multihop_valid_paths 不用区别直接保存； 
    # ratio_valid_invalid 不用区别直接保存； aggregator_type 直接保存
    if args.path_encoder == "NOUSE":
        if args.GTDM == "USEGTDM":
            model_sub_dir = '{}-{}{}{}{}-{}'.format(
                dataset,
                args.model,
                reverse_edge_tag,
    ...
    if args.path_encoder == "USE":
        model_sub_dir = '{}-{}{}{}{}-{}-{}-{}-{}-{}-{}'.format(
            ...
            hyperparam_sig,
            args.GTDM,
            args.path_encoder_type,
            args.number_multihop_valid_paths,
            args.ratio_valid_invalid,
