...
parser.add_argument('--path_encoder',type=str, default='NOUSE', help='USE 使用路径编码器，NOUSE 不使用路径编码器')
parser.add_argument('--path_encoder_type',type=str, default='Attn',help='使用编码器的类型')
parser.add_argument('--GTDM',type=str, default="USEGTDM",help='USEGTDM在推理过程中掩码真实数据; NOGTDM在推理过程中不使用真实掩码') 
parser.add_argument('--number_multihop_valid_paths', type=int, default=20) # 多跳路径：demo有效路径路径数目 10  ； 2 10 20
parser.add_argument('--ratio_valid_invalid',type=int,default=3) # demo 无效路径的数目 10 * 3=30   ；  1,2,3 比例；
# hm=LR(W1(he+hm))+LR(W2(he*w)); hm+relu(he*w);  relu(hm*w)+he
parser.add_argument('--aggregator_type',type=str,default='14',help='使用的聚合器类型') #在最后在测试不同的聚合器。目前先不考虑；聚合器类型；
parser.add_argument('--xavier_init', type=bool, default=True) # umls ： 注意力的静态融合   动态权重衰减融合  正则化   
parser.add_argument('--use_regu', type=bool,default=False)  # 正则化是否使用
parser.add_argument('--use_decay',type=bool,default=False)  # 动态衰减是否使用
# 在这里先固定采样数目进行对比；
